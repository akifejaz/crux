...
ytshorts::Config default_cfg() { ytshorts::Config c; c.max_clips = 6; return c; }

ytshorts::Heatmap make_flat_high(double val, double bin_s) {
    ytshorts::Heatmap h; h.bin_seconds = bin_s;
...
ytshorts::Heatmap single_peak_short() {
    // Karate-Kid-like: cluster 71..78 peaks near bin 75.
    ytshorts::Heatmap h; h.bin_seconds = 2.4;
    for (std::size_t i = 0; i < ytshorts::kBinCount; ++i) {
...
TEST_CASE("planner: short video with single region emits one clamped clip") {
    ...
    double dur = ytshorts::kBinCount * h.bin_seconds; // 240s
...
TEST_CASE("planner: flat-high (music) profile flagged") {
    ...
    auto p = ytshorts::planner::plan(det, h, 280.0, {}, cfg);
...
TEST_CASE("planner: greedy min-gap keeps regions well-separated") {
    ...
    ytshorts::Heatmap h; h.bin_seconds = 30.0;    // 3000s video, 30s bins
    for (std::size_t i = 0; i < ytshorts::kBinCount; ++i) {
    ...
    auto cfg = default_cfg();
    auto det = ytshorts::detector::detect(h, 3000.0, cfg);
    auto p = ytshorts::planner::plan(det, h, 3000.0, {}, cfg);
...
TEST_CASE("planner: clip windows clamp by shift near end of video") {
    ytshorts::Heatmap h; h.bin_seconds = 30.0;
    for (std::size_t i = 0; i < ytshorts::kBinCount; ++i) {
        h.bins[i].start_sec = i * 30.0;
        h.bins[i].end_sec   = (i + 1) * 30.0;
        h.bins[i].value = 0.05;

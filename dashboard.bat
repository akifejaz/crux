@echo off
REM Launches the ytshorts dashboard server and opens the default browser.
REM Usage:
...
set "EXE=build\Release\ytshorts_server.exe"
if not exist "%EXE%" set "EXE=build\ytshorts_server.exe"
if not exist "%EXE%" goto :missing_exe

if not exist "dashboard\index.html" goto :missing_dashboard

echo Starting ytshorts dashboard on http://127.0.0.1:%PORT%/ ...
...
:missing_exe
echo [error] ytshorts_server.exe not built yet.
